...
Verify CI needs **no repository database secret**, including on ordinary fork pull requests. After checkout and Node 22 setup, [tests/ci-database.mjs](tests/ci-database.mjs) generates a fresh 32-byte random password, registers GitHub masks before use, then starts only its named job-owned container from the exact pinned PostgreSQL 15/17 and UTC/America_New_York allowlists. The password reaches Docker through a named environment variable, never an argument literal. [tests/configure-test-database.mjs](tests/configure-test-database.mjs) percent-encodes it, masks the complete URL, and writes a single `TEST_DATABASE_URL` line to private `GITHUB_ENV`; no secret output or artifact is created. Service identity remains `audit` / `medapp_audit`, bound only at `127.0.0.1:55439:5432`. Bounded health checks gate integration; failure/cancellation cleanup removes only the matching job-owned container, not existing containers or volumes. Public run IDs identify resources, never credentials. No trust authentication, published password, privileged PR trigger, or owner-secret setup is required. Manual URL configuration still accepts a privately supplied `TEST_DATABASE_PASSWORD`; the workflow does not consume it. See [the CI database fix and local probe](docs/verification/ci-database-fix.md); hosted success still requires a new observed run.
